...
float boostTrackCoordinateToCM(float coordinate, const Parameters &par)
{
   if (par.yBoost == 0 || par.isPP)
      return coordinate;
   if (par.isPPb)
      return coordinate - par.yBoost;
   return -(coordinate + par.yBoost);
}

float getTrackLongitudinalCoordinate(ZHadronMessenger *b, const Parameters &par, unsigned long trackIndex)
{
   float coordinate = (*b->trackEta)[trackIndex];

   if (par.usePionTrackY)
   {
      if (b->trackY == nullptr || trackIndex >= b->trackY->size())
      {
         cerr << "Error! UsePionTrackY=true requires the skimmed trackY branch." << endl;
         exit(1);
      }

      // trackY in the skim is filled from a TLorentzVector built with the pion mass.
      coordinate = (*b->trackY)[trackIndex];
      coordinate = boostTrackCoordinateToCM(coordinate, par);
   }
   ...
   for (unsigned long i = iStart; i < iEnd; i++) { 
      ...
      for (unsigned int nMix = 0; nMix < targetMix; nMix++) {
         ...
         for (unsigned long j = 0; j < (par.mix ? MMix->trackPhi->size() : MZSignal->trackPhi->size()); j++) {
            ...
            float trackLongitudinal = par.mix ? getTrackLongitudinalCoordinate(MMix, par, j)
                                              : getTrackLongitudinalCoordinate(MZSignal, par, j);
            float trackDeta  = fabs(trackLongitudinal - zY);
