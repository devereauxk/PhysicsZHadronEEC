...
int main(int argc, char *argv[])
{
   ...
   string outputBase = CL.Get("OutputBase", "plots/pionTrackStudy/systematic");
   ...
   {
      cerr << "Unable to open pion comparison inputs" << endl;
      return 1;
   }
   ...
   for(const string &observable : observables)
   {
      ...
      if(variationPPbFile != nullptr && variationPBPFile != nullptr)
         variation = BuildCombinedResultHistogram(*variationPPbFile, *variationPBPFile, observable, trackRange, observable + "_Pion");
      else if(variationFile != nullptr)
         variation = LoadResultHistogram(*variationFile, observable, trackRange, observable + "_Pion");

      if(nominal == nullptr || variation == nullptr)
      {
         cerr << "Missing " << observable << " histogram for pion comparison" << endl;
         delete nominal;
         delete variation;
         return 1;
      }
      ...
      TCanvas canvas(("CanvasPionTrackY" + observable).c_str(), "", 600, 600);
      TPad *pad = (TPad *)plotCMSDiff(
         histograms, ("PionTrackY_" + observable).c_str(), {"Nominal", "Pion-mass track y"},
         {cmsBlue, cmsRed}, {0, 0},
         ...
         "pion - nominal", differenceRange.first, differenceRange.second,
         0,
         false, false, true,
         0.56
      );
      ...
      DrawKinematicLabels(pad, "Nominal vs pion-mass track y", zptRange, trackRange);
