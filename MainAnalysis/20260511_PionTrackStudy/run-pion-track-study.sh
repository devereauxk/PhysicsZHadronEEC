...
if [ "$DOPPB" = "1" ]; then
   run_ppb_chain "pPb" true "${OFFICIAL_DATAINPUT_PPB}" "${ZWeightFile_PPb}" "${RWeightFile_PPb}" "${VZWeightFile_PPb}" "${PPB_TAG}"
fi

if [ "$DOPBP" = "1" ]; then
   run_ppb_chain "PbP" false "${OFFICIAL_DATAINPUT_PBP}" "${ZWeightFile_PbP}" "${RWeightFile_PbP}" "${VZWeightFile_PbP}" "${PPB_TAG}"
fi
