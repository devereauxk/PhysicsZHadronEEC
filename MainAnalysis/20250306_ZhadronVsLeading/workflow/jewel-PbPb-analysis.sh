...
for zpt_range in "${ZPT_RANGES[@]}"; do
   ...
   hadd -f "plots/$1jewelPbPb030_ZPT${min_zpt}_${max_zpt}-result.root" \
         $(for pt_range in "${PT_RANGES[@]}"; do
            echo "output/$1jewelPbPb030_ZPT${min_zpt}_${max_zpt}-${pt_range}-result.root"
         done)
   hadd -f "plots/$1jewelPbPb030_ZPT${min_zpt}_${max_zpt}-nosub.root" \
         $(for pt_range in "${PT_RANGES[@]}"; do
            echo "output/$1jewelPbPb030_ZPT${min_zpt}_${max_zpt}-${pt_range}-nosub.root"
         done)
done
