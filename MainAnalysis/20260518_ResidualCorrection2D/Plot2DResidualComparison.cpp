...
int main(int argc, char *argv[])
{
   ...
   string outputBase = CL.Get("OutputBase", "plots/2DResidualComparison");
   ...
   {
      cerr << "Unable to open comparison inputs" << endl;
      return 1;
   }
   ...
   for(const string &observable : observables)
   {
      ...
      if(variationPPbFile != nullptr && variationPBPFile != nullptr)
         variation = BuildCombinedResultHistogram(*variationPPbFile, *variationPBPFile, observable, trackRange, observable + "_2DResidual");
      else if(variationFile != nullptr)
         variation = LoadResultHistogram(*variationFile, observable, trackRange, observable + "_2DResidual");

      if(nominal == nullptr || variation == nullptr)
      {
         cerr << "Missing " << observable << " histogram for comparison" << endl;
         delete nominal;
         delete variation;
         return 1;
      }
      ...
      TCanvas canvas(("Canvas2DResidual" + observable).c_str(), "", 600, 600);
      TPad *pad = (TPad *)plotCMSDiff(
         histograms, ("2DResidual_" + observable).c_str(), {"1D residual (official)", "2D residual"},
         {cmsBlue, cmsRed}, {0, 0},
         ...
         "2D - 1D", differenceRange.first, differenceRange.second,
         0,
         false, false, true,
         0.56
      );
      ...
      DrawKinematicLabels(pad, "1D vs 2D residual correction", zptRange, trackRange);
