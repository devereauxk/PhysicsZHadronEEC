#!/bin/bash
# Study 11: nThread=40 comparison run (full pPb+Pbp inclusive).
# Used as the "nominal" nThread choice to compare against nThread=60.
...
echo "=== pPb nThread=40 ==="
./system-analysis.sh "pPb_trkResidual_${TAG}_12x12_nthread40" \
    "${COMMON[@]}" \
    --IsPPb true \
...
echo "=== Pbp nThread=40 ==="
./system-analysis.sh "PbP_trkResidual_${TAG}_12x12_nthread40" \
    "${COMMON[@]}" \
    --IsPPb false \
...
rm -f "$CONFIG"
echo "=== Done: nthread40 analysis complete ==="
