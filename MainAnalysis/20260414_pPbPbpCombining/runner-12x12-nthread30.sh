#!/bin/bash
# Study 11: nThread=30 comparison run (full pPb+Pbp inclusive).
...
echo "=== pPb nThread=30 ==="
./system-analysis.sh "pPb_trkResidual_${TAG}_12x12_nthread30" \
    "${COMMON[@]}" \
    --IsPPb true \
...
echo "=== Pbp nThread=30 ==="
./system-analysis.sh "PbP_trkResidual_${TAG}_12x12_nthread30" \
    "${COMMON[@]}" \
    --IsPPb false \
...
rm -f "$CONFIG"
echo "=== Done: nthread30 analysis complete ==="
