#!/bin/bash
# Study 8 extension: Nmix=40 sensitivity.
...
echo "=== pPb Nmix=40 ==="
./system-analysis.sh "pPb_trkResidual_${TAG}_12x12_nmix40" \
    "${COMMON[@]}" \
    --IsPPb true \
...
echo "=== Pbp Nmix=40 ==="
./system-analysis.sh "PbP_trkResidual_${TAG}_12x12_nmix40" \
    "${COMMON[@]}" \
    --IsPPb false \
...
rm -f "$CONFIG"
echo "=== Done: nmix40 analysis complete ==="
