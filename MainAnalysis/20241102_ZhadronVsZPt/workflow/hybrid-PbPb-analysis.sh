...
source config.sh
COMMON_ARGS="--UseLeadingTrk $UseLeadingTrk --Input mergedSample/hybrid_PbPb030-v9.root --MixFile mergedSample/hybrid_PbPb030-v9.root --IsPP false --IsGenZ true --nMix 10 --MinHiBin 0 --MaxHiBin 600000"
...
for zpt_range in "${ZPT_RANGES[@]}"; do
   ...
   hadd -f "plots/$1hybridPbPb030_ZPT${min_zpt}_${max_zpt}-result.root" \
         $(for pt_range in "${PT_RANGES[@]}"; do
            echo "output/$1hybridPbPb030_ZPT${min_zpt}_${max_zpt}-${pt_range}-result.root"
         done)
   hadd -f "plots/$1hybridPbPb030_ZPT${min_zpt}_${max_zpt}-nosub.root" \
         $(for pt_range in "${PT_RANGES[@]}"; do
            echo "output/$1hybridPbPb030_ZPT${min_zpt}_${max_zpt}-${pt_range}-nosub.root"
         done)
done
