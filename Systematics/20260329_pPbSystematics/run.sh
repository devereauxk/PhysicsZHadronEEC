...
for SYSTEM in "${SYSTEMS_ARRAY[@]}"; do
   if [ "$SYSTEM" = "pp" ]; then
    ...
    DIFFERENCE_FAMILIES="${USER_DIFFERENCE_FAMILIES:-$DEFAULT_DIFF_FAMILIES}"

    for ZPT in "${ZPT_ARRAY[@]}"; do
       if [ "$SYSTEM" = "pPbPbp" ]; then
          NOMINAL_FILE_PP="$ANALYSISDIR/pp_trkResidual_${OFFICIAL_TAG_PP}_ZPT${ZPT}-result.root"
          ...
          TRACK_SELECTION_FILES_PP="$ANALYSISDIR/pp_trkResidual_${OFFICIAL_TAG_PP}_Loose_ZPT${ZPT}-result.root,$ANALYSISDIR/pp_trkResidual_${OFFICIAL_TAG_PP}_Tight_ZPT${ZPT}-result.root"
          ...
          TRACK_CORRECTION_FILES_PP="$ANALYSISDIR/pp_trkResidual_${OFFICIAL_TAG_PP}_TrackCorrection0p976_ZPT${ZPT}-result.root,$ANALYSISDIR/pp_trkResidual_${OFFICIAL_TAG_PP}_TrackCorrection1p024_ZPT${ZPT}-result.root"
          ...
          MUON_REJECTION_FILES_PP="$ANALYSISDIR/pp_trkResidual_${OFFICIAL_TAG_PP}_IsMuTaggedFalse_ZPT${ZPT}-result.root"
          ...
          PU_PP_FILE_PP="$ANALYSISDIR/pp_trkResidual_${OFFICIAL_TAG_PP}_IsPURejectTrue_ZPT${ZPT}-result.root"
          ...
          SCALE_FACTOR_FILES_PP="$ANALYSISDIR/pp_trkResidual_${OFFICIAL_TAG_PP}_MuVar0_ZPT${ZPT}-result.root,$ANALYSISDIR/pp_trkResidual_${OFFICIAL_TAG_PP}_MuVar1_ZPT${ZPT}-result.root,$ANALYSISDIR/pp_trkResidual_${OFFICIAL_TAG_PP}_MuVar2_ZPT${ZPT}-result.root,$ANALYSISDIR/pp_trkResidual_${OFFICIAL_TAG_PP}_MuVar3_ZPT${ZPT}-result.root"
          ENERGY_EXTRAPOLATION_FILES_PP="$ANALYSISDIR/pp_trkResidual_${OFFICIAL_TAG_PP}_EEPrivate_ZPT${ZPT}-result.root"
        else
