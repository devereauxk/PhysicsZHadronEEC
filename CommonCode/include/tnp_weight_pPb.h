#ifndef tnp_weight_pPb_h
#define tnp_weight_pPb_h

#include <cmath>
#include <limits>

// pPb high-pT Tag-and-Probe weights from
// CMS-HIN-dilepton/MuonAnalysis-TagAndProbe, branch 80X_HI,
// file macros/tnp_weight.h.
...
inline double tnp_weight_pPb_nan()
{
...
{
   if(tnp_weight_pPb_valid_muon(pt, eta) == false)
      return tnp_weight_pPb_nan();

   if(idx > 0)
      return tnp_weight_muid_ppb(pt, eta, -1);
   if(idx < 0)
      return tnp_weight_muid_ppb(pt, eta, -2);
   return tnp_weight_muid_ppb(pt, eta, 0);
}

double tnp_weight_L3Mu12_double_pPb(double pt1, double eta1, double pt2, double eta2, int idx = 0)
{
   if(tnp_weight_pPb_valid_muon(pt1, eta1) == false || tnp_weight_pPb_valid_muon(pt2, eta2) == false)
      return tnp_weight_pPb_nan();

   double EffMC1 = tnp_weight_trg_ppb(eta1, 300);
   double EffMC2 = tnp_weight_trg_ppb(eta2, 300);
   double EffData1 = tnp_weight_trg_ppb(eta1, 200);
   double EffData2 = tnp_weight_trg_ppb(eta2, 200);

   if(idx != 0)
   {
      int Variation = (idx > 0) ? -1 : -2;
      EffData1 = tnp_weight_trg_ppb(eta1, Variation) * EffMC1;
      EffData2 = tnp_weight_trg_ppb(eta2, Variation) * EffMC2;
   }

   double EffEventData = 1 - (1 - EffData1) * (1 - EffData2);
   double EffEventMC = 1 - (1 - EffMC1) * (1 - EffMC2);
   return EffEventData / EffEventMC;
}

#endif // tnp_weight_pPb_h
