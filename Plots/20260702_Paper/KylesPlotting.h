    ...
    int panelMode = 0, double borderOverride = -1)
{
    ...
    float legHeight = std::max(0.15f, 0.04f * textScale * (float)hists.size() + 0.03f);
    TLegend* leg = new TLegend(xLegend, yLeg, xLegend + 0.23, yLeg + legHeight);
    leg->SetBorderSize(0);
    leg->SetFillStyle(0);
    leg->SetTextFont(42);
    ...
    for (int i = 0; i < (int)hists.size(); i++) {
        ...
        if (linestyles[i] == 0) hist->SetLineStyle(1);
        else if (linestyles[i] >= 10) hist->SetLineStyle(linestyles[i] - 10);
        else if (linestyles[i] > 0) hist->SetLineStyle(linestyles[i]);
        hist->SetMarkerColor(markercolors[i]);
        hist->SetMarkerStyle(markerstyles[i]);
        hist->SetStats(0);
        ...
        if (isMarker) {
            ...
            if (linestyles[i] >= 10) {
                ...
                TGraph* gLine = new TGraph(nB);
                for (int b = 1; b <= nB; b++)
                    gLine->SetPoint(b-1, hist->GetBinCenter(b), hist->GetBinContent(b));
                gLine->SetLineColor(linecolors[i]);
                gLine->SetLineStyle(linestyles[i] - 10);
                gLine->SetLineWidth(2);
                gLine->Draw("L SAME");
                leg->AddEntry(gLine, labels[i].c_str(), "l");
            } else {
                hist->Draw("HIST SAME");
        ...
        if (i != baseline) {
            ...
            if (isMarker) {
                ...
                if (linestyles[i] >= 10) {
                    ...
                    int nB = hDiff->GetNbinsX();
                    TGraph* gDiffLine = new TGraph(nB);
                    for (int b = 1; b <= nB; b++)
                        gDiffLine->SetPoint(b-1, hDiff->GetBinCenter(b), hDiff->GetBinContent(b));
                    gDiffLine->SetLineColor(linecolors[i]);
                    gDiffLine->SetLineStyle(linestyles[i] - 10);
                    gDiffLine->SetLineWidth(2);
                    gDiffLine->Draw("L SAME");
                } else {
                    TString diffDrawOption = drawStat ? "E1" : ((linestyles[i] > 0) ? "HIST" : "P");
                ...
                if (bottomSyst != nullptr) {
                    ...
                    else if (linestyles[i] >= 10) { /* TGraph already drawn, no redraw needed */ }
                    else if (linestyles[i] > 0) hDiff->Draw("HIST SAME");
                    else hDiff->Draw("P SAME");
                }

                if (linestyles[i] >= 10) { /* TGraph handles its own redraw */ }
                else if (drawStat) hDiff->Draw("E1 SAME");
                else if (linestyles[i] > 0) hDiff->Draw("HIST SAME");
                else hDiff->Draw("P SAME");
            }
