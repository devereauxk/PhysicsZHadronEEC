...
- **20260506 ten-bin note-result convention**: the shifted modified-bin `10 x 10` surface is now an older semi-official study surface, not the maintained note-facing default. Its tags remain `EEV5_ZV9_trkV27_nmix10_bin10x10shifted_20260506` (pp) and `ZV9_trkV27_nmix10_bin10x10shifted_20260506` (pPb/PbP), refreshed via `MainAnalysis/20241102_ZhadronVsZPt/result-study-bin10x10shifted-20260506.sh`, `MainAnalysis/20241102_ZhadronVsZPt/systematics-bin10x10-20260506.sh`, `Systematics/20260329_pPbSystematics/run-bin10x10-20260506.sh`, and `Plots/20260213_Central/plot-central-combined-bin10x10-20260502.sh`; keep the extra exploratory slides in `central_combined_bin10x10_20260502.pdf` on the older central/stat-only study outputs.
- **20260506 ten-bin Overleaf convention**: the shifted modified-bin `10 x 10` Overleaf copy pattern remains available for archival comparisons, but the maintained note `Results` section should now prefer the newer `12 x 12` modified-bin figure set instead of refreshing these `10 x 10` assets.
- **20260507 twelve-bin note-result convention**: the maintained modified-bin note surface uses the tags `EEV5_ZV9_trkV27_nmix10_bin12x12_20260507` (pp) and `ZV9_trkV27_nmix10_bin12x12_20260507` (pPb/PbP). Refresh the approved inclusive bin `ZPT0_500`, `trkPT0.5_15` plus the six scan bins `ZPT={0_30,30_500}` x `trkPT={0.5_2,2_4,4_15}` through `MainAnalysis/20241102_ZhadronVsZPt/result-study-bin12x12-20260507.sh`, `MainAnalysis/20241102_ZhadronVsZPt/systematics-bin12x12-20260507.sh`, `Systematics/20260329_pPbSystematics/run-bin12x12-20260507.sh`, and `Plots/20260213_Central/plot-central-combined-bin12x12-20260507.sh`, then stage the seven-slide presentation as `Plots/20260213_Central/presentations/central_combined_bin12x12_20260507.pdf`.
- **20260507 twelve-bin Overleaf convention**: note-facing `12 x 12` updates should copy the inclusive PDFs `all_ZPT0_500_trkPT0.5_15_*_bin12x12_20260507-{DeltaPhi,DeltaEta}-result.pdf` plus the dedicated scan composites `Plots/20260213_Central/overleaf_pdf_composites/overleaf_result_scan_bin12x12_{deltaphi,deltaeta}_combined.pdf` into `~/OverleafZHadronInPPb/figures/result/`, preserving those source basenames exactly and replacing the older `10 x 10` modified-bin additions in `src/results.tex`.
- **20260506 projection-window convention**: maintained result-stage projections must now be hard-coded by surface, not auto-detected. The official `20 x 20` path keeps the frozen windows `DeltaPhi: x bins 0..10` and `DeltaEta: y bins 6..10`. The shifted modified-bin `10 x 10` path uses `DeltaEta` bins `-4..4` and `DeltaPhi` bins `-3#pi/5..7#pi/5`, requires exact axis edges at `DeltaEta = 0, 4` and `DeltaPhi = 0, #pi`, and projects with `DeltaPhi: x bins 6..10` (`0..4` in `DeltaEta`) and `DeltaEta: y bins 4..8` (`0..#pi` in `DeltaPhi`). The maintained modified-bin `12 x 12` path uses `DeltaEta` bins `-4..4` and `DeltaPhi` bins `-#pi/2..3#pi/2`, requires exact axis edges at `DeltaEta = 0, 4` and `DeltaPhi = 0, #pi/2, #pi`, and projects with `DeltaPhi: x bins 7..12` (`0..4` in `DeltaEta`) and `DeltaEta: y bins 4..6` (`0..#pi/2` in `DeltaPhi`).
...
- **20260506 jackknife note-plot convention**: maintained note-facing result plots and pPb-vs-PbP comparison plots should use jackknife statistical uncertainties by default. `MainAnalysis/20241102_ZhadronVsZPt/central.sh` and the maintained `10 x 10` result-study runner should pass `--UseJackknife true` on corrected-data result legs, the pPb-vs-PbP compatibility recalculation lives in `MainAnalysis/20260506_Jackknife/`, and `Plots/20260213_Central/plot-central-overlay-PPbPbP.sh` should default to the finalized note bins (`0_30`, `30_500`, `0_500` crossed with `0.5_2`, `2_4`, `4_15`, `0.5_15`) rather than the stale exploratory ranges.
