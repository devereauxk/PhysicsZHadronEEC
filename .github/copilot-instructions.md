...
- `MainAnalysis/20260414_pPbPbpCombining/`: self-contained pPb/PbP comparison sandbox. Observables: trkPt, trkEta, ZPt, Zy, Mult, DEta, DPhi, DEtaDPhi 2D ratio. No DR. `trackCharge` is empty in V0.2/V0.3 skims (ReduceForest never populates it) so charge-based observables are unavailable. Uses `runner.sh` (6 correction levels × 2 PU families = 24 runs), `run_z_correction.sh` (3-iteration pPb→PbP Z correction), `runner_with_zcorr.sh` (Z-corrected runs), and `_chunk${N}.root` naming for hadd safety. Presentations in `presentations/`.
