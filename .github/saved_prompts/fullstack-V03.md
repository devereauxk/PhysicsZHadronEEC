...
## official skims sets
The new pp skims have been completed and are available at
...
As well as the new pPb and Pbp skims are here
`SampleGeneration/20250929_ReducedTreePA/V0.3/PPbData_Reco.root` for ppb data
`SampleGeneration/20250929_ReducedTreePA/V0.3/PPbMC_Reco.root` for ppb MC reco
`SampleGeneration/20250929_ReducedTreePA/V0.3/PPbMC_Gen.root` for ppb MC gen
`SampleGeneration/20250929_ReducedTreePA/V0.3/PbPData_Reco.root` for pbp data
`SampleGeneration/20250929_ReducedTreePA/V0.3/PbPMC_Reco.root` for pbp MC reco
`SampleGeneration/20250929_ReducedTreePA/V0.3/PbPMC_Gen.root` for pbp MC gen
We will refer to these at the V0.3 skims. We will continue to use the EPOS files from the same paths as before as well as the private pp inputs. Assume all files have been correctly labelled for their respective collision orientation.

## new dictionary
Following the same proceadure we did when we migrated from the last OfficialWeightDictionary, you should copy the OfficialWeightDictionary as it is to a different file for bookkeeping, call it OfficialWeightDictionary_SkimV02.sh. And start a new OfficialWeightDictionary but witout any correction factor paths filled in. The intention is we can fall back to the old corrections if needed. Start by filling in the paths to the new official V0.3 skims. Also increment the new official tags to
```
...
Use todays date 20260407 for the new corrections. Use the blank slate OfficialWeightDictionary.sh as a laundry list for the correction factors to recalculate. Recalculate factors this time for pp, pPb and Pbp. Calculate the factors one-by-one in the same was as was done for the V0.2 skims, only moving onto the next factor when the previous one is done running. Calculate factors in this order
 - VZ correction
 - Z correction
 - track residual correction
 - energy extrapolation (data-driven, pp only)
...
For each level of corrections recalculated, the associated closure plots should be reproduced and copied to the overleaf. Reference the overleaf for the VZ, Z, track residual, energy extrapolation and other closure plots to remake and copy. A full list is also given here for a full list of plots to refresh with the new results and copy over. Use the same kinematic selection (zpt and track pt) as is done in the overleaf for each set of plots.
 - HLT trigger : remake these for pPb and Pbp seperately. Also make a new one for pp as the HLT hist information is now available. Add the pp plot to the same figure as the pPb and Pbp trigger curves in the trigger efficiency section. Make these in `Plots/20260326_HLTCurves` and copy over to the note.
 - VZ correction weight ratios for pp, pPb, Pbp as well as closure for VZ reweighting. Make these in `Plots/20251001_pPbVZReweighting` and copy over to the note.
 ...
 - all closure plots for the central values in MC before and after background substraction. Make these with `Plots/20260120_CentralClosure/`. At this step an audit should be made for the closure at MC-level after background subtraction. These plots should close reasonable well. If they do not then bug hunt and report the issues. If they are silly then fix and continue the plotting campaign.
 - combining pPb and Pbp data plots. Make these at `Plots/20260213_Central/plot_central_overlay_PPbPbP.cpp`. 
 - all systematics plots should be made new. We use the standard that pPb+Pbp plots for systematics should always be made for the "combined" sets. We also have the information for pp as detailed in the "new systematics" section of this note. And should now make standalone plots for pp for track selection systematic and muon-track tagging. Make these plots and add them to their respective sections in the note. Also calculate the new total systematic uncertainties for pp and pPb+Pbp. pp should display curves now for (TrackSelection, TrackCorrection, MuonRejection, PU, ScaleFactor, EnergyExtrapolation, Total) and pPb+Pbp should display (TrackSelection, TrackCorrection, MuonRejection, PU, ScaleFactor, Total). Use the same kinematics selection Z pt 5-500 and trk pT 0.5-500 for all the systematic plots. Make these at `Systematics/
...
We will most likely repeat this same exact procedure several times in the future. Produce a .md file which is most parsable to reviewer agents that summarizes this task and that I can feed back to the reviewer agent easily just changing the path of the updated skim, so I don't have to rewrite all this info in the future.
