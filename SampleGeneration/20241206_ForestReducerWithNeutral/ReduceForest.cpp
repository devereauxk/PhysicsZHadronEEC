...
int main(int argc, char *argv[])
{
   ...
   bool StorePFNeutral                = CL.GetBool("StorePFNeutral", false);
   ...
   for(string InputFileName : InputFileNames)
   {
      ...
      for(int iE = 0; iE < EntryCount; iE++)
      {
         ...
         if(StorePFNeutral == true)
         {
            int NPF = 0;
            if(DoGenLevel == true)   NPF = MGen.Mult;
            else                     NPF = MPF.ID ? MPF.ID->size() : 0;

            for(int iPF = 0; iPF < NPF; iPF++)
            {
               if(DoGenLevel == true)
               {
                  if(MGen.DaughterCount->at(iPF) > 0)
                     continue;
               }

               double Eta = DoGenLevel ? MGen.Eta->at(iPF) : MPF.Eta->at(iPF);
               double Phi = DoGenLevel ? MGen.Phi->at(iPF) : MPF.Phi->at(iPF);
               double PT  = DoGenLevel ? MGen.PT->at(iPF) : MPF.PT->at(iPF);
               int Charge = DoGenLevel ? MGen.Charge->at(iPF) : PFCharge(MPF.ID->at(iPF));
               int ID = DoGenLevel ? MGen.ID->at(iPF) : MPF.ID->at(iPF);
               ...
               MZHadron.neutralPt->push_back(PT);
               MZHadron.neutralEta->push_back(Eta);
               MZHadron.neutralPhi->push_back(Phi);
               MZHadron.neutralPDGId->push_back(ID);
            }
         }
...
int PFCharge(int ID)
{
   if(ID == 0)   // unknown
      return 999;
   if(ID == 1 || ID == 2 || ID == 3)   // charged hadron, electron, muon
      return 1;
   if(ID == 4 || ID == 5)   // photon & neutral hadron
      return 0;
   if(ID == 6 || ID == 7)   // HF stuff
      return 999;
   return 999;
}
